...
HParams = namedtuple('HParams',
                     ...
                     'num_softmax_samples, vocab_size, num_gpus')
    ...
    def run_train_step(self, sess, article_batch, abstract_batch, targets,
        ...
        return sess.run(to_return,
                        feed_dict={self._articles: article_batch,
                                   ...
                                   #self._embedding_placeholder: wordEmbedding
                                  })
    ...
    def _add_seq2seq(self):
        ...
        for layer_i in xrange(hps.enc_layers):
            with tf.variable_scope('encoder%d'%layer_i), tf.device(
                self._next_device()):
                cell_fw = tf.contrib.rnn.LSTMCell(hps.num_hidden,initializer=tf.random_uniform_initializer(-0.1, 0.1, seed=123),state_is_tuple=True)
                cell_bw = tf.contrib.rnn.LSTMCell(hps.num_hidden,initializer=tf.random_uniform_initializer(-0.1, 0.1, seed=113),state_is_tuple=True)
        ...
        with tf.variable_scope('decoder'), tf.device(self._next_device()):
            ...
            cell = tf.contrib.rnn.LSTMCell(hps.num_hidden, initializer=tf.random_uniform_initializer(-0.1, 0.1, seed=113), state_is_tuple=True)
